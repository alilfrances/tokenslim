...
!`node -e 'const fs=require("node:fs"),path=require("node:path"),cp=require("node:child_process");const roots=[process.env.TOKENSLIM_PLUGIN_ROOT,process.env.CLAUDE_PLUGIN_ROOT,process.env.PLUGIN_ROOT,process.cwd()].filter(Boolean);const script=roots.map(r=>path.join(r,"scripts/discover.mjs")).find(fs.existsSync);if(!script){console.log("tokenslim: unable to locate scripts/discover.mjs. Reinstall or update the plugin.");process.exit(0)}process.exit(cp.spawnSync(process.execPath,[script,...process.argv.slice(1)],{stdio:"inherit"}).status??0)' "$ARGUMENTS"`
